...
/*
 * initGL: create()
 * paintGL: bindFrameBuffer to bind the frame buffer ->
 *     glViewport(0,0,this->width() * this->devicePixelRatio(), this->height() * this->devicePixelRatio());
      // Clear the screen so that we only see newly drawn images
       glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
 * draw terrain (first pass)
 * call: glBindFramebuffer(GL_FRAMEBUFFER, this->defaultFramebufferObject());
 *     glViewport(0,0,this->width() * this->devicePixelRatio(), this->height() * this->devicePixelRatio());
      // Clear the screen
       glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
       FB::bindToTextureSlot()
* unif sampler2d of postshader set to same texture slot: 0
* draw the m_quad: quadrangle
*/